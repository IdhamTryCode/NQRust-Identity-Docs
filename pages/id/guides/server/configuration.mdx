...
3. Opsi yang didefinisikan dalam file `conf/nqrust-identity.conf`, atau dalam file konfigurasi yang dibuat pengguna.
...
Anda dapat menggunakan placeholder untuk menghitung nilai spesifik lingkungan dari variabel lingkungan di dalam file `nqrust-identity.conf` dengan menggunakan sintaks `${ENV_VAR}`:
...
Secara default, server selalu mengambil opsi konfigurasi dari file `conf/nqrust-identity.conf`. Untuk instalasi baru, file ini hanya memegang pengaturan komentar sebagai ide tentang apa yang ingin Anda atur saat berjalan dalam produksi.
...
Mengatur opsi itu membuat NQRust-Identity membaca konfigurasi dari file yang ditentukan bukan dari `conf/nqrust-identity.conf`.
...
- buat entri di file `nqrust-identity.conf` Anda yang merujuk pada variabel lingkungan yang Anda pilih. contohnya `kc.log-level-package.class_name=${CLASS_NAME_LEVEL}`. Lihat lebih lanjut tentang merujuk variabel lingkungan di [Format untuk merujuk variabel lingkungan](#_format_for_referencing_environment_variables).
- atau dalam situasi di mana mengubah `nqrust-identity.conf` mungkin tidak mudah, Anda dapat menggunakan pasangan variabel lingkungan `KC_UNIQUEIFIER=value` dan `KCKEY_UNIQUEIFIER=key`, misalnya `KC_MYKEY=debug` dan `KCKEY_MYKEY=log-level-package.class_name`, atau `KC_LOG_LEVEL_PACKAGE_CLASS_NAME=debug` dan `KCKEY_LOG_LEVEL_PACKAGE_CLASS_NAME=log-level-package.class_name`
...
Secara default, opsi konfigurasi contoh untuk mode produksi dikomentari dalam file `conf/nqrust-identity.conf` default. Opsi ini memberi Anda gambaran tentang konfigurasi utama yang perlu dipertimbangkan saat menjalankan NQRust-Identity dalam produksi.
...
1. Atur opsi build untuk vendor basis data PostgreSQL menggunakan perintah build
  ...
  bin/kc.[sh|bat] build --db=postgres
  ```
2. Atur opsi konfigurasi runtime untuk postgres di file `conf/nqrust-identity.conf`.
