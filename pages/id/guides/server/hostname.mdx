...
Seperti yang ditunjukkan dalam contoh sebelumnya,skema dan port tidak secara eksplisit diperlukan. Dalam kasus seperti itu, NQRust-Identity secara otomatis menangani aspek-aspek ini. Sebagai contoh, server akan dapat diakses di `https://my.nqrust-identity.org:8443` dalam contoh yang diberikan. Namun, proxy balik biasanya akan mengekspos NQRust-Identity di port default, seperti `443`. Dalam hal ini, lebih diinginkan untuk menentukan URL penuh di opsi `hostname` daripada menjaga bagian-bagian URL dinamis. Server dapat kemudian dijalankan dengan:
...
Dengan cara ini, aplikasi Anda, yang disebut sebagai klien, dapat terhubung dengan NQRust-Identity melalui jaringan lokal Anda, sementara server tetap dapat diakses secara publik di `https://my.nqrust-identity.org`.
...
Hasil dari konfigurasi ini adalah bahwa Anda dapat terus mengakses NQRust-Identity di `https://my.nqrust-identity.org` melalui HTTPS, sementara proxy berinteraksi dengan instance menggunakan HTTP dan port `8080`.
...
Dalam hal ini, skema, port, dan jalur konteks diuraikan secara dinamis dari header X-Forwarded-*, sementara hostname didefinisikan secara statis sebagai `my.nqrust-identity.org`.
...
Hal ini memungkinkan Anda untuk mengakses NQRust-Identity di `https://my.nqrust-identity.org` dan Konsol Admin di `https://admin.my.nqrust-identity.org:8443`, sementara backend tetap menggunakan `https://my.nqrust-identity.org`.
...
Catatan bahwa opsi `hostname` harus diatur ke URL. Untuk informasi lebih lanjut, lihat bagian [Validasi](#_validations) di bawah.
...
Sekali lagi, opsi `hostname` harus diatur ke URL. Untuk informasi lebih lanjut, lihat bagian [Validasi](#_validations) di bawah.
...
Setelah NQRust-Identity berhasil dijalankan, buka peramban Anda dan arahkan ke: `http://mynqrust-identity:8080/realms/<your-realm>/hostname-debug`
