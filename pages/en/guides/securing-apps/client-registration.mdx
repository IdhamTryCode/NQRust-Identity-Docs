...
In order for an application or service to utilize NQRust-Identity it has to register a client in NQRust-Identity.
An admin can do this through the admin console (or admin REST endpoints), but clients can also register themselves through the NQRust-Identity client registration service.

The Client Registration Service provides built-in support for NQRust-Identity Client Representations, OpenID Connect Client Meta Data and SAML Entity Descriptors.
...
The bearer token can be issued on behalf of a user or a Service Account. The following permissions are required to invoke the endpoints:
...
If you are using a bearer token to create clients it’s recommend to use a token from a Service Account with only the `create-client` role.
...
## NQRust-Identity Representations

The `default` client registration provider can be used to create, retrieve, update and delete a client.
It uses NQRust-Identity Client Representation format which provides support for configuring clients exactly as they can be configured through the admin
...
## NQRust-Identity adapter configuration
...
This means that for the JavaScript adapter you can load the client configuration directly from NQRust-Identity using the above URL.
...
For those operations the NQRust-Identity representation endpoints should be used.
When creating a client a NQRust-Identity Client Representation is returned with details about the created client, including a registration access token.
...
<Callout type="info">
  The current plans are for the Client Registration Policies to be removed in favor of the Client Policies feature.
  Client Policies are more flexible and support more use case.
  For example the case of enforcing client URIs like the JWKS to prevent [SSRF attacks](https://owasp.org/www-community/attacks/Server_Side_Request_Forgery) allowing only trusted patterns or domains.
</Callout>
...
In NQRust-Identity admin console, you can click to `Client Registration` tab and then `Client Registration Policies` sub-tab. Here you will see what policies
...
<Callout type="info">
  NQRust-Identity normalizes loopback addresses to `localhost` to avoid platform-specific
