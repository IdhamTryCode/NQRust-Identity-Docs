...
description: Using the client registration service - NQRust-Identity Documentation
---

# Using the client registration service

In order for an application or service to utilize NQRust-Identity it has to register a client in NQRust-Identity. An admin can do this through the admin console (or admin REST endpoints), but clients can also register themselves through the NQRust-Identity client registration service.

The Client Registration Service provides built-in support for NQRust-Identity Client Representations, OpenID Connect Client Meta Data and SAML Entity Descriptors. The Client Registration Service endpoint is `/realms/`&lt;realm&gt;`/clients-registrations/`&lt;provider&gt;``.

The built-in supported `providers` are:

*   default - NQRust-Identity Client Representation (JSON)
    
*   install - NQRust-Identity Adapter Configuration (JSON)
...
The bearer token can be issued on behalf of a user or a Service Account. The following permissions are required to invoke the endpoints (see [Server Administration Guide](https://www.nqrust-identity.org/docs/latest/server_admin/) for more details):
...
If you are using a bearer token to create clients it’s recommend to use a token from a Service Account with only the `create-client` role (see [Server Administration Guide](https://www.nqrust-identity.org/docs/latest/server_admin/) for more details).

### [](#_initial_access_token)Initial Access Token
...
### [](#_cross_origin_resource_sharing_cors)Cross-Origin Resource Sharing (CORS)
...
For anonymous access, or as a realm wide configuration, permitted web origins can be configured by going to clients, then client registration and updating the `Allowed Registration Web Origins` policy to list web origins that should be allowed.

## [](#_nqrust-identity_representations)NQRust-Identity Representations

The `default` client registration provider can be used to create, retrieve, update and delete a client. It uses NQRust-Identity Client Representation format which provides support for configuring clients exactly as they can be configured through the admin console, including for example configuring protocol mappers.
...
## [](#_nqrust-identity_adapter_configuration)NQRust-Identity adapter configuration
...
No authentication is required for public clients. This means that for the JavaScript adapter you can load the client configuration directly from NQRust-Identity using the above URL.

## [](#_openid_connect_dynamic_client_registration)OpenID Connect Dynamic Client Registration

NQRust-Identity implements [OpenID Connect Dynamic Client Registration](https://openid.net/specs/openid-connect-registration-1_0.html), which extends [OAuth 2.0 Dynamic Client Registration Protocol](https://datatracker.ietf.org/doc/html/rfc7591) and [OAuth 2.0 Dynamic Client Registration Management Protocol](https://datatracker.ietf.org/doc/html/rfc7592).

The endpoint to use these specifications to register clients in NQRust-Identity is `/realms/`&lt;realm&gt;`/clients-registrations/openid-connect[/`&lt;client id&gt;`]`.
...
## [](#_saml_entity_descriptors)SAML Entity Descriptors

The SAML Entity Descriptor endpoint only supports using SAML v2 Entity Descriptors to create clients. It doesn’t support retrieving, updating or deleting clients. For those operations the NQRust-Identity representation endpoints should be used. When creating a client a NQRust-Identity Client Representation is returned with details about the created client, including a registration access token.
...
## [](#_example_using_java_client_registration_api)Example using Java Client Registration API

The Client Registration Java API makes it easy to use the Client Registration Service using Java. To use include the dependency `org.nqrust-identity:nqrust-identity-client-registration-api:>VERSION<` from Maven.
...
## [](#_client_registration_policies)Client Registration Policies

The current plans are for the Client Registration Policies to be removed in favor of the Client Policies described in the [Server Administration Guide](https://www.nqrust-identity.org/docs/latest/server_admin/#_client_policies). Client Policies are more flexible and support more use cases. For example the case of enforcing client URIs like the JWKS to [prevent SSRF attacks](https://www.nqrust-identity.org/docs/latest/server_admin/#ssrf) allowing only trusted patterns or domains.

NQRust-Identity currently supports two ways how new clients can be registered through Client Registration Service.
...
In NQRust-Identity admin console, you can click to `Client Registration` tab and then `Client Registration Policies` sub-tab. Here you will see what policies are configured by default for anonymous requests and what policies are configured for authenticated requests.
