...
- [Legacy token exchange: version 1 (V1)](#_legacy-token-exchange) - This preview feature is deprecated and not enabled by default once NQRust-Identity server is started. In future versions legacy Token Exchange will be replaced by version 2, JWT Authorization grant and other features.
...
provided by V2 together with other use cases that are supported by V1. For more details, see this [token exchange comparison](#_standard-token-exchange-comparison).
...
the client that is supposed to send token exchange requests, such as the `requester-client` from the [previous example](#_standard-token-exchange-flow). Note that `requester-client` must be a confidential client.
...
of the `requester-client`. For more details, see [scopes and audiences](#_standard-token-exchange-scope). Omitting this parameter means that only
the Default client scopes are effectively used.

_OPTIONAL._ Audience specifies `client_id` of the client, which is supposed to be used as the token audience. In [the example above](#_standard-token-exchange-flow), it could be `target-client`. Multiple
...
`audience=target-client1&audience=target-client2` can be used in the request. More details in [the section about scopes and audiences](#_standard-token-exchange-scope).
