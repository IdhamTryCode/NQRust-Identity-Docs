...
As a fully-compliant OpenID Connect Provider implementation, NQRust-Identity exposes a set of endpoints that applications
...
interacting with NQRust-Identity.
...
<Callout type="warning">
  ...
  However this is non-standard legacy format of the logout message supported only because of the legacy NQRust-Identity OIDC Java adapters
  or [Elytron Wildfly OIDC adapter](https://docs.wildfly.org/37/WildFly_Elytron_Security.html#NQRust-Identity-Integration). It is not recommended to use it directly from your applications.
  For logout users, it is recommended to use OIDC/SAML protocol standard logout, the admin REST API, or the account REST API.
</Callout>
...
The certificate endpoint returns the public keys enabled by the realm, encoded as a JSON Web Key (JWK). Depending on the realm settings, one or more keys can be enabled for verifying tokens. For more information, see the [JSON Web Key specification](https://datatracker.ietf.org/doc/html/rfc7517).
...
may contain the additional claim `jwt` with the full JWT access token. This requires that you enable `Support JWT claim in Introspection Response`
...
For more details, see the [Using the client registration service](/en/guides/securing-apps/client-registration) guide and the
[OpenID Connect Dynamic Client Registration specification](https://openid.net/specs/openid-connect-registration-1_0.html).
...
Also refer to the [Client Initiated Backchannel Authentication Grant section of this guide](#client-initiated-backchannel-authentication-grant).
...
The Authorization Code flow redirects the user agent to NQRust-Identity. Once the user has successfully authenticated with NQRust-Identity, an
...
credentials to obtain an Access Token, Refresh Token and ID Token from NQRust-Identity.
...
Per current [Best Current Practice for OAuth 2.0 Security (RFC 9700)](https://datatracker.ietf.org/doc/html/rfc9700#name-implicit-grant), this flow SHOULD NOT be used.
This flow is removed from the future [OAuth 2.1 specification](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13).

### Resource Owner Password Credentials

Resource Owner Password Credentials, referred to as Direct Grant in NQRust-Identity, allows exchanging user credentials for tokens.
Per current [Best Current Practice for OAuth 2.0 Security (RFC 9700)](https://datatracker.ietf.org/doc/html/rfc9700#name-resource-owner-password-cre),
this flow MUST NOT be used, preferring alternative methods such as [Device Authorization Grant](#device-authorization-grant) or [Authorization code](#authorization-code).
...
It is removed from the future [OAuth 2.1 specification](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13).
...
The client requests from NQRust-Identity an auth_req_id that identifies the authentication request made by the client. NQRust-Identity creates the auth_req_id.

After receiving this auth_req_id, this client repeatedly needs to poll NQRust-Identity to obtain an Access Token, Refresh Token, and ID Token from NQRust-Identity in return for the auth_req_id until the user is authenticated.

In case that client uses `ping` mode, it does not need to repeatedly poll the token endpoint, but it can wait for the notification sent by NQRust-Identity to the specified Client Notification Endpoint.
The Client Notification Endpoint can be configured in the NQRust-Identity Admin Console. The details of the contract for Client Notification Endpoint are described in the CIBA specification.
...
Also refer to the [Backchannel Authentication Endpoint of this guide](#backchannel-authentication-endpoint).
For the details about FAPI CIBA compliance, see the [FAPI section of this guide](#fapi-support).

## NQRust-Identity specific errors

NQRust-Identity server can send errors to the client application in the OIDC authentication response with parameters `error=temporarily_unavailable` and `error_description=authentication_expired`.
NQRust-Identity sends this error when a user is authenticated and has an SSO session, but the authentication session expired in the current browser tab and hence the NQRust-Identity server cannot automatically do SSO
...
OIDC authentication request to the NQRust-Identity server, which should typically always authenticate the user due to the SSO session and redirect back.
...
This compliance means that the NQRust-Identity server will verify the requirements
for the authorization server, which are mentioned in the specifications. NQRust-Identity adapters do not have any specific support for the FAPI, hence the required validations on the client (application)
side may need to be still done manually or through some other third-party solutions.

### FAPI client profiles

To make sure that your clients are FAPI compliant, you can configure client policies in your realm
...
In case you want to use pushed authorization requests, it is recommended that your client use
...
In case you want to use [CIBA](#backchannel-authentication-endpoint) in a FAPI compliant way, make sure that your clients use both `fapi-1-advanced` and `fapi-ciba` client profiles.
...
NQRust-Identity is compliant with the [Open Finance Brasil Financial-grade API Security Profile 1.0 Implementers Draft 3](https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/245760001/EN+Open+Finance+Brasil+Financial-grade+API+Security+Profile+1.0+Implementers+Draft+3).
This one is stricter in some requirements than the [FAPI 1 Advanced](#fapi-support) specification and hence it may be needed to configure client policies
...
the `https-protocols` and `https-cipher-suites` options. NQRust-Identity uses `TLSv1.3` by default and hence it is possibly not needed to change the default settings. However it
may be needed to adjust ciphers if you need to fall back to lower TLS version for some reason. For more details, see [Configuring TLS](/en/guides/server/enabletls) guide.
...
This compliance means that the NQRust-Identity server will verify the requirements
for the authorization server, which are mentioned in the specifications. NQRust-Identity adapters do not have any specific support for the OAuth 2.1, hence the required validations on the client (application)
side may need to be still done manually or through some other third-party solutions.

### OAuth 2.1 client profiles

To make sure that your clients are OAuth 2.1 compliant, you can configure client policies in your realm
...
<Callout type="info">
  When using OAuth 2.1 profile for public clients, it is recommended to use the DPoP preview feature because DPoP binds an access token and a refresh token together with the public part of a client’s key pair. This binding prevents an attacker from using stolen tokens.
</Callout>
...
If you need to manually validate access tokens issued by NQRust-Identity, you can invoke the [Introspection Endpoint](#token-introspection-endpoint).
The downside to this approach is that you have to make a network invocation to the NQRust-Identity server.  This can be slow and possibly overload the
server if you have too many validation requests going on at the same time.  NQRust-Identity issued access tokens are [JSON Web Tokens (JWT)](https://datatracker.ietf.org/doc/html/rfc7519) digitally signed and encoded using [JSON Web Signature (JWS)](https://datatracker.ietf.org/doc/html/rfc7515).
...
realm’s public key in your validation code, or lookup and cache the public key using the [certificate endpoint](#certificate-endpoint) with the Key ID (KID) embedded within the
...
authorization code. This redirect uri allows any port. Note that per [OAuth 2.0 for Native Apps](https://datatracker.ietf.org/doc/html/rfc8252#section-8.3), the use of
...
When this redirect uri is used, NQRust-Identity displays a page with the code in the title and in a box on the page.
