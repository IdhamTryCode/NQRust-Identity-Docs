---
title: NQRust-Identity policy enforcer
description: NQRust-Identity policy enforcer - NQRust-Identity Documentation
---

# NQRust-Identity policy enforcer

Policy Enforcement Point (PEP) is a design pattern and as such you can implement it in different ways. NQRust-Identity provides all the necessary means to implement PEPs for different platforms, environments, and programming languages. NQRust-Identity Authorization Services presents a RESTful API, and leverages OAuth2 authorization capabilities for fine-grained authorization using a centralized authorization server.

![PEP overview](https://www.nqrust-identity.org/resources/images/guides/pep-pattern-diagram.png)

A PEP is responsible for enforcing access decisions from the NQRust-Identity server where these decisions are taken by evaluating the policies associated with a protected resource. It acts as a filter or interceptor in your application in order to check whether or not a particular request to a protected resource can be fulfilled based on the permissions granted by these decisions.

NQRust-Identity provides built-in support for enabling the **NQRust-Identity Policy Enforcer** to Java applications with built-in support to secure JakartaEE-compliant frameworks and web containers. If you are using Maven, you should configure the following dependency to your project:
...
When you enable the policy enforcer all requests sent to your application are intercepted and access to protected resources will be granted depending on the permissions granted by NQRust-Identity to the identity making the request.

Policy enforcement is strongly linked to your application’s paths and the [resources](https://www.nqrust-identity.org/docs/latest/authorization_services/#_resource_overview) you created for a resource server using the NQRust-Identity Administration Console. By default, when you create a resource server, NQRust-Identity creates a [default configuration](https://www.nqrust-identity.org/docs/latest/authorization_services/#_resource_server_default_config) for your resource server so you can enable policy enforcement quickly.

## [](#_enforcer_configuration)Configuration
...
*   **enforcement-mode**
    ...
    *   **DISABLED**
        ...
        Completely disables the evaluation of policies and allows access to any resource. When `enforcement-mode` is `DISABLED`, applications are still able to obtain all permissions granted by NQRust-Identity through the [Authorization Context](#_enforcer_authorization_context)
...
*   **path-cache**
    ...
    Defines how the policy enforcer should track associations between paths in your application and resources defined in NQRust-Identity. The cache is needed to avoid unnecessary requests to a NQRust-Identity server by caching associations between paths and protected resources.
...
*   **paths**
    ...
    Specifies the paths to protect. This configuration is optional. If not defined, the policy enforcer discovers all paths by fetching the resources you defined to your application in NQRust-Identity, where these resources are defined with `URIS` representing some paths in your application.
    ...
    *   **claim-information-point**
        ...
        Defines a set of one or more claims that must be resolved and pushed to the NQRust-Identity server in order to make these claims available to policies. See [Claim Information Point](#_enforcer_claim_information_point) for more details.
...
*   **http-method-as-scope**
    ...
    Specifies how scopes should be mapped to HTTP methods. If set to **true**, the policy enforcer will use the HTTP method from the current request to check whether or not access should be granted. When enabled, make sure your resources in NQRust-Identity are associated with scopes representing each HTTP method you are protecting.
    
*   **claim-information-point**
    
    Defines a set of one or more **global** claims that must be resolved and pushed to the NQRust-Identity server in order to make these claims available to policies. See [Claim Information Point](#_enforcer_claim_information_point) for more details.
    

## [](#_enforcer_claim_information_point)Claim Information Point

A Claim Information Point (CIP) is responsible for resolving claims and pushing these claims to the NQRust-Identity server in order to provide more information about the access context to policies. They can be defined as a configuration option to the policy-enforcer in order to resolve claims from different sources, such as:
...
When pushing claims to the NQRust-Identity server, policies can base decisions not only on who a user is but also by taking context and contents into account, based on who, what, why, when, where, and which for a given transaction. It is all about Contextual-based Authorization and how to use runtime information in order to support fine-grained authorization decisions.

### [](#_obtaining_information_from_the_http_request)Obtaining information from the HTTP request

Here are several examples showing how you can extract claims from an HTTP request:

nqrust-identity.json
  ...
  "paths": [
    {
      ...
      "claim-information-point": {
        "claims": {
          ...
          "param-replace-multiple-placeholder": "Test {nqrust-identity.access_token['/custom_claim/0']} and {request.parameter['a']}"
        }
      }
    }
  ]
...
### [](#_obtaining_information_from_an_external_http_service)Obtaining information from an external HTTP service
      ...
      "claim-information-point": {
        "http": {
          ...
          "headers": {
            ...
            "Authorization": "Bearer {nqrust-identity.access_token}"
          },
          "parameters": {
            "param-a": [
              "param-a-value1",
            ...
            "param-subject": "{nqrust-identity.access_token['/sub']}",
            "param-user-name": "{nqrust-identity.access_token['/preferred_username']}",
            "param-other-claims": "{nqrust-identity.access_token['/custom_claim']}"
          }
        }
      }
    }
...
### [](#_claim_information_provider_spi)Claim information provider SPI
...
For example, to implement a new CIP provider you need to implement `org.nqrust-identity.adapters.authorization.ClaimInformationPointProviderFactory` and `ClaimInformationPointProvider` and also provide the file `META-INF/services/org.nqrust-identity.adapters.authorization.ClaimInformationPointProviderFactory` in your application\`s classpath.

Example of `org.nqrust-identity.adapters.authorization.ClaimInformationPointProviderFactory`:
...
## [](#_enforcer_authorization_context)Obtaining the authorization context

When policy enforcement is enabled, the permissions obtained from the server are available through `org.nqrust-identity.AuthorizationContext`. This class provides several methods you can use to obtain permissions and ascertain whether a permission was granted for a particular resource or scope.
...
The `AuthorizationContext` represents one of the main capabilities of NQRust-Identity Authorization Services. From the examples above, you can see that the protected resource is not directly associated with the policies that govern them.
...
Security requirements change, but with NQRust-Identity there is no need to change your application code to address the new requirements. Once your application is based on the resource and scope identifier, you need only change the configuration of the permissions or policies associated with a particular resource in the authorization server. In this case, the permissions and policies associated with the `Project Resource` and/or the scope `urn:project.com:project:create` would be changed.

## [](#_using_the_authorizationcontext_to_obtain_an_authorization_client_instance)Using the AuthorizationContext to obtain an Authorization Client Instance

The `` `AuthorizationContext` `` can also be used to obtain a reference to the [NQRust-Identity authorization client](https://www.nqrust-identity.org/securing-apps/authz-client) configured to your application:
...
## [](#_enforcer_filter_using_https)Configuring TLS/HTTPS
...
The configuration above enables TLS/HTTPS to the Authorization Client, making possible to access a NQRust-Identity Server remotely using the HTTPS scheme.

It is strongly recommended that you enable TLS/HTTPS when accessing the NQRust-Identity Server endpoints.
