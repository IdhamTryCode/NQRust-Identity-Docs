...
Policy Enforcement Point (PEP) is a design pattern and as such you can implement it in different ways. NQRust-Identity provides all the necessary means
to implement PEPs for different platforms, environments, and programming languages. NQRust-Identity Authorization Services presents a RESTful API,
and leverages OAuth2 authorization capabilities for fine-grained authorization using a centralized authorization server.

![PEP overview](/images/securing-apps/policy-enforcer-1.png)

A PEP is responsible for enforcing access decisions from the NQRust-Identity server where these decisions are taken by evaluating the policies
...
NQRust-Identity provides built-in support for enabling the **NQRust-Identity Policy Enforcer** to Java applications with built-in support to secure JakartaEE-compliant frameworks and web containers.
...
depending on the permissions granted by NQRust-Identity to the identity making the request.

Policy enforcement is strongly linked to your application’s paths and the resources you created for a resource server using the NQRust-Identity Administration Console. By default,
when you create a resource server, NQRust-Identity creates a default configuration for your resource server so you can enable policy enforcement quickly.
...
applications are still able to obtain all permissions granted by NQRust-Identity through the [Authorization Context](#_enforcer_authorization_context)
- **on-deny-redirect-to**
...
- **path-cache**
  ...
  Defines how the policy enforcer should track associations between paths in your application and resources defined in NQRust-Identity. The cache is needed to avoid
unnecessary requests to a NQRust-Identity server by caching associations between paths and protected resources.
...
check whether or not access should be granted. When enabled, make sure your resources in NQRust-Identity are associated with scopes representing each HTTP method you are protecting.
- **claim-information-point**
...
A Claim Information Point (CIP) is responsible for resolving claims and pushing these claims to the NQRust-Identity server
...
When pushing claims to the NQRust-Identity server, policies can base decisions not only on who a user is but also by taking
...
and `ClaimInformationPointProvider` and also provide the file `META-INF/services/org.nqrust-identity.adapters.authorization.ClaimInformationPointProviderFactory`
...
When policy enforcement is enabled, the permissions obtained from the server are available through `org.nqrust-identity.AuthorizationContext`.
...
The `AuthorizationContext` can also be used to obtain a reference to the [NQRust-Identity authorization client](/en/guides/securing-apps/authz-client) configured to your application:
...
NQRust-Identity Server remotely using the HTTPS scheme.
