...
In case the external identity provider is another NQRust-Identity server or realm, Standard Token Exchange can be used to obtain such a token (see [Configuring and using token exchange](/en/guides/securing-apps/token-exchange#standard-token-exchange) for more information). When both sides are NQRust-Identity realms, the idea can be summarized in two basic points:
