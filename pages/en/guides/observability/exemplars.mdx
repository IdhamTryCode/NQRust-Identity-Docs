...
To connect a metric to a recorded trace, there is the concept of [exemplars](https://grafana.com/docs/grafana/latest/fundamentals/exemplars/).

Once exemplars are set up, NQRust-Identity reports metrics with their last recorded trace as an exemplar.
...
See the guide HTTP metrics for details on this metric.
- `nqrust-identity_credentials_password_hashing_validations_total`

See the guide Self-provided metrics  for details on this metric.
...
1. Enable metrics for NQRust-Identity as described in guide [Gaining insights with metrics](/en/guides/observability/configuration-metrics).
2. Enable tracing for NQRust-Identity as described in guide [Root cause analysis with tracing](/en/guides/observability/tracing).
...
2. For test purposes, record all traces by setting the tracing ration to `1.0`.
See [Root cause analysis with tracing](/en/guides/observability/tracing#sampling) for recommended sampling settings in production systems.
