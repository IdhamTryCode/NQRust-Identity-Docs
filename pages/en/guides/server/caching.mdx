...
NQRust-Identity is designed for high availability and multi-node clustered setups. The current distributed cache implementation is built on top of [Infinispan](https://infinispan.org), a high-performance, distributable in-memory data grid.

## [](#_enable_distributed_caching)Enable distributed caching
...
By default, caches use the `jdbc-ping` stack which is based upon a TCP transport and uses the configured database to track nodes joining the cluster. NQRust-Identity allows you to either choose from a set of pre-defined default transport stacks, or to define your own custom stack, as you will see later in this guide.
...
When you start NQRust-Identity in development mode, by using the `start-dev` command, NQRust-Identity uses only local caches and distributed caches are completely disabled by implicitly setting the `--cache=local` option. The `local` cache mode is intended only for development and testing purposes.

## [](#_configuring_caches)Configuring caches

NQRust-Identity provides a regular [Infinispan configuration file](https://infinispan.org/docs/stable/titles/configuring/configuring.html) located at `conf/cache-ispn.xml`. This file contains the default configuration used for the cache-container and JGroups transport.
...
### [](#_cache_types_and_defaults)Cache types and defaults
...
These in-memory caches for user sessions and client sessions are limited to, by default, 10000 entries per node which reduces the overall memory usage of NQRust-Identity for larger installations. The internal caches will run with only a single owner for each cache entry.
...
The `loginFailures` distributed cache is used to track data about failed login attempts. This cache is needed for the Brute Force Protection feature to work in a multi-node NQRust-Identity setup.
...
### [](#_volatile_user_sessions)Volatile user sessions

By default, regular user sessions are stored in the database and loaded on-demand to the cache. It is possible to configure NQRust-Identity to store regular user sessions in the cache only and minimize calls to the database.
...
When using volatile user sessions, the cache is the source of truth for user and client sessions. NQRust-Identity automatically adjusts the number of entries that can be stored in memory, and increases the number of copies to prevent data loss.
...
### [](#_configuring_cache_maximum_size)Configuring cache maximum size
...
### [](#_specify_your_own_cache_configuration_file)Specify your own cache configuration file
...
### [](#_modifying_cache_configuration_defaults)Modifying cache configuration defaults
...
While overriding the default cache configurations via XML is technically possible, it is not supported. This is only recommended for advanced use-cases where the default cache configurations are proven to be problematic. The only supported way to change the default cache configurations is via the `cache-...` options.
...
### [](#_cli_options_for_remote_server)CLI options for remote server

For configuration of NQRust-Identity server for high availability and multi-node clustered setup there was introduced following CLI options `cache-remote-host`, `cache-remote-port`, `cache-remote-username` and `cache-remote-password` simplifying configuration within the XML file. Once any of the declared CLI parameters are present, it is expected there is no configuration related to remote store present in the XML file.

#### [](#_connecting_to_an_insecure_infinispan_server)Connecting to an insecure Infinispan server

Disabling security is not recommended in production!

In a development or test environment, it is easier to start an unsecured Infinispan server. For these use case, the CLI options `cache-remote-tls-enabled` disables the encryption (TLS) between NQRust-Identity and Infinispan. NQRust-Identity will fail to start if the Infinispan server is configured to accept only encrypted connections.

The CLI options `cache-remote-username` and `cache-remote-password` are optional and, if not set, NQRust-Identity will connect to the Infinispan server without presenting any credentials. If the Infinispan server has authentication enabled, NQRust-Identity will fail to start.

## [](#cache-topology)Topology aware data distribution

Configuring NQRust-Identity to be aware of your network topology, increases data availability in the presence of hardware failures, as Infinispan is able to ensure that data is distributed correctly. For example, if `num_owners=2` is configured for a cache, it will ensure that the two owners are not stored on the same node when possible.
...
If your NQRust-Identity cluster is deployed between different datacenters, use this option to ensure the data replicas are stored in a different datacenter. It prevents data loss if a datacenter goes offline or fails.

Use the SPI option `spi-cache-embedded--default--site-name` (or environment variable `KC_SPI_CACHE_EMBEDDED__DEFAULT__SITE_NAME`). The value itself is not important, but each datacenter must have a unique value.

For example: `--spi-cache-embedded--default--site-name=site-1`

Rack name

If your NQRust-Identity cluster is running in different racks on your datacenter, set this option to ensure the data replicas are stored in a different physical rack. It prevents data loss if a rack is suddenly disconnected or fails.

Use the SPI option `spi-cache-embedded--default--rack-name` (or environment variable `KC_SPI_CACHE_EMBEDDED__DEFAULT__RACK_NAME`). The value itself is not important, but each rack must have a unique value.

For example: `--spi-cache-embedded--default--rack-name=rack-1`

Machine name

If you have multiple NQRust-Identity instances running on the same physical machine (using virtual machines or containers for example), use this option to ensure the data replicas are stored in different physical machines. It prevents data loss against a physical machine failure.

Use the SPI option `spi-cache-embedded--default--machine-name` (or environment variable `KC_SPI_CACHE_EMBEDDED__DEFAULT__MACHINE_NAME`). The value itself is not important, but each machine must have a unique value.

For example: `--spi-cache-embedded--default--machine-name=machine-1`

The NQRust-Identity Operator automatically configure the machine name based on the Kubernetes node. It ensures that if multiple pods are scheduled on the same node, data replicas are still replicated across distinct nodes when possible. We recommend to set up anti-affinity rules and/or topology spread constraints to prevent multiple Pods from being scheduled on the same node, further reducing the risk of a single node failure causing data loss.

## [](#_transport_stacks)Transport stacks

Transport stacks ensure that NQRust-Identity nodes in a cluster communicate in a reliable fashion. NQRust-Identity supports a wide range of transport stacks:
...
### [](#_available_transport_stacks)Available transport stacks
...
When using the `tcp`, `udp` or `jdbc-ping-udp` stack, each cluster must use a different multicast address and/or port so that their nodes form distinct clusters. By default, NQRust-Identity uses `239.6.7.8` as multicast address for `jgroups.mcast_addr` and `46655` for the multicast port `jgroups.mcast_port`.

Use `-D`&lt;property&gt;`=`&lt;value&gt;`` to pass the properties via the `JAVA_OPTS_APPEND` environment variable or in the CLI command.

**Additional Stacks**

It is recommended to use one of the stacks available above. Additional stacks are provided by Infinispan, but it is outside the scope of this guide how to configure them. Please refer to [Setting up Infinispan cluster transport](https://infinispan.org/docs/stable/titles/embedding/embedding.html#cluster-transport) and [Customizing JGroups stacks](https://infinispan.org/docs/stable/titles/embedding/embedding.html#customizing-jgroups-stacks_cluster-transport) for further documentation.

## [](#_securing_transport_stacks)Securing transport stacks
...
With TLS enabled, NQRust-Identity auto-generates a self-signed RSA 2048 bit certificate to secure the connection and uses TLS 1.3 to secure the communication. The keys and the certificate are stored in the database so they are available to all nodes. By default, the certificate is valid for 60 days and is rotated at runtime every 30 days. Use the option `cache-embedded-mtls-rotation-interval-days` to change this.

### [](#_running_inside_a_service_mesh)Running inside a service mesh

When using a service mesh like Istio, you might need to allow a direct mTLS communication between the NQRust-Identity Pods to allow for the mutual authentication to work. Otherwise, you might see error messages like `JGRP000006: failed accepting connection from peer SSLSocket` that indicate that a wrong certificate was presented, and the cluster will not form correctly.
...
*   Apply the following configuration to allow direct communication.
    ...
    **1**
    ...
    Update the labels to match your NQRust-Identity deployment.
...
Failure detection by protocol `FD_SOCK2`. It listens to the abrupt closing of a socket to suspect a NQRust-Identity server failure. The `jgroups.fd.port-offset` property defines the offset from the `cache-embedded-network-bind-port` option or `jgroups.bind.port` property. By default, the offset is set to 50000, making the failure detection port 57800.
...
## [](#network-bind-address)Network bind address
...
Picks a global IP address if available. If not available, it falls back to `SITE_LOCAL`.
...
Picks an address that matches a pattern against the interface name. For example, `match-interface:tun0` or `match-interface:eth.\*`.

`match-address:`&lt;regex&gt;``

Picks an address that matches a pattern against the host address. For example, `match-address:192.168.\*`.

`match-host:`&lt;regex&gt;``

Picks an address that matches a pattern against the host name. For example, `match-host:linux.\*`.
...
## [](#_running_instances_on_different_networks)Running instances on different networks

If you run NQRust-Identity instances on different networks, for example behind firewalls or in containers, the different instances will not be able to reach each other by their local IP address. In such a case, set up a port forwarding rule (sometimes called “virtual server”) to their local IP address.
...
Port that other instances in the NQRust-Identity cluster should use to contact this node.

`cache-embedded-network-external-address`

IP address that other instances in the NQRust-Identity should use to contact this node.

## [](#_verify_cluster_and_network_health)Verify cluster and network health

This section provides methods to verify that your NQRust-Identity cluster has formed correctly and that network communication between instances is functioning as expected. It is crucial to perform these checks after deployment to ensure high availability and data consistency.

To verify if the cluster is formed properly, check one of these locations:

*   Admin UI
    
    Access the NQRust-Identity Web UI, typically available at `https://`&lt;your-host&gt;`/admin/master/console/#/master/providers`. Under the **Provider Info** section, locate the **connectionsInfinispan** entry. Click on **Show more** to expand its details. You should find information about the cluster status and the health of individual caches.
    
    ![Infinispan Cluster Information in Web UI](https://www.nqrust-identity.org/resources/images/guides/server/infinispan_info.png)
    
*   Logs
    
    Infinispan logs a cluster view every time a new instance joins or leaves the cluster. Search for log entries with the ID `ISPN000094`.
    ...
    This log entry indicates that the cluster named "ISPN" currently has 2 nodes: `node1-26186` and `node2-37007`. The `(2)` confirms the total number of nodes in the cluster.
    
*   Metrics
    
    NQRust-Identity exposes Infinispan metrics via a Prometheus endpoint, which can be visualized in tools like Grafana. The metric `vendor_cluster_size` shows the current number of instances in the cluster. You should verify that this metric matches the expected number of running instances configured in your cluster.
    
    Refer to [Clustering metrics](https://www.nqrust-identity.org/observability/metrics-for-troubleshooting-clustering-and-network#_cluster_size) for more information.
    

## [](#_exposing_metrics_from_caches)Exposing metrics from caches
...
For more details about how to enable metrics, see [Gaining insights with metrics](https://www.nqrust-identity.org/observability/configuration-metrics).
...
### [](#_embedded_cache)Embedded Cache
