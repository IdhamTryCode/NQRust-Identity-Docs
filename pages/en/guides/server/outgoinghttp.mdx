...
See [Configuring trusted certificates](/en/guides/server/identity-truststore) for how
to configure a NQRust-Identity Truststore so that NQRust-Identity is able to perform outgoing requests using TLS.
...
Specify proxy configurations for outgoing HTTP requests. For more details, see [Proxy mappings for outgoing HTTP requests](#proxy-mappings-for-outgoing-http-requests).
