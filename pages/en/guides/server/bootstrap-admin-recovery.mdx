...
## [](#_a_temporary_admin_account)A temporary admin account

A user or service admin account created using one of the methods described below is **temporary**. This means the account should exist only for the duration necessary to perform operations needed to gain permanent and more secure admin access. After that, the account needs to be removed manually. Various UI/UX elements, such as the Administration Console warning banner, labels, and log messages, will indicate to a NQRust-Identity administrator that the account is temporary.

## [](#_bootstrapping_a_temporary_admin_account_at_nqrust-identity_startup)Bootstrapping a temporary admin account at NQRust-Identity startup

NQRust-Identity `start` and `start-dev` commands support options for bootstrapping both temporary admin users and admin service accounts. These options are standard configuration options, so they can be specified in any of the [configuration sources](https://www.nqrust-identity.org/server/configuration#_configuring_sources_for_nqrust-identity) such as environment variables or CLI parameters. For instance, the following examples demonstrate how to use the `start` and `start-dev` commands with CLI parameters to bootstrap a temporary admin user and an admin service account, respectively:
...
The username or client ID values can be omitted; see the [Default values](#_default_values) section below for more information.

The purpose of these options is solely for bootstrapping temporary admin accounts. These accounts will be created only during the initial start of the NQRust-Identity server when the master realm doesn’t exist yet. The accounts are always created in the master realm. For recovering lost admin access, use the dedicated command described in the sections below.

## [](#_bootstrapping_an_admin_user_or_service_account_using_the_dedicated_command)Bootstrapping an admin user or service account using the dedicated command

The `bootstrap-admin` command can be executed even before the first-ever start of NQRust-Identity. Bear in mind that all the NQRust-Identity nodes need to be stopped prior to using this command. Its execution will trigger the creation of the initial master realm, and as a result, the startup options to bootstrap the admin user and service account will be ignored later when the server is started for the first time.

Additionally, it is strongly recommended to use the dedicated command with the same options that the NQRust-Identity server is started with (e.g., `db` options).
...
### [](#_create_an_admin_user)Create an admin user
...
### [](#_create_a_service_account)Create a service account
...
## [](#_disable_the_parameters_prompt)Disable the parameters prompt
...
This creates a temporary admin user with the default username without prompting for confirmation. For more information, see the [Default values](#_default_values) section above.
