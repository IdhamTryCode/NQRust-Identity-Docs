...
If you have built an optimized version of NQRust-Identity with the `build` command as outlined in [Configuring NQRust-Identity](/en/guides/server/configuration), use the command line option `--optimized` to have NQRust-Identity skip the build check for a faster startup time.
