...
<Callout type="info">
  ...
  Consistency of an export is not guaranteed unless all NQRust-Identity nodes are stopped prior to running the export.
...
<Callout type="info">
  ...
  It is recommended that all NQRust-Identity nodes are stopped prior to using the `kc.[sh|bat] export` command.
  This ensures that the results will have no consistency issues with user or realm modifications during the export.

  It is required that all NQRust-Identity nodes are stopped prior to using the `kc.[sh|bat] import` command with the override option.
...
When using the `export` and the `import` commands below, NQRust-Identity needs to know how to connect to the database where the information about realms, clients, users and other entities is stored.
As described in [Configuring NQRust-Identity](/en/guides/server/configuration) that information can be provided as command line parameters, environment variables or a configuration file.
...
As default, NQRust-Identity will re-build automatically for the `export` and `import` commands if it detects a change of a build time parameter.
