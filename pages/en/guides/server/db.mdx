...
## [](#_supported_databases)Supported databases
...
## [](#_installing_a_database_driver)Installing a database driver
...
### [](#_installing_the_oracle_database_driver)Installing the Oracle Database driver
...
1.  Download the `ojdbc17` and `orai18n` JAR files from one of the following sources:
    ...
    1.  **Zipped JDBC driver and Companion Jars** version 23.6.0.24.10 from the [Oracle driver download page](https://www.oracle.com/database/technologies/appdev/jdbc-downloads.html).
        
    2.  Maven Central via `[ojdbc17](https://repo1.maven.org/maven2/com/oracle/database/jdbc/ojdbc17/23.6.0.24.10/ojdbc17-23.6.0.24.10.jar)` and `[orai18n](https://repo1.maven.org/maven2/com/oracle/database/nls/orai18n/23.6.0.24.10/orai18n-23.6.0.24.10.jar)`.
...
3.  When running containers: Build a custom NQRust-Identity image and add the JARs in the `providers` folder. When building a custom image for the Operator, those images need to be optimized images with all build-time options of NQRust-Identity set.
    ...
    ADD --chown=nqrust-identity:nqrust-identity --chmod=644 https://repo1.maven.org/maven2/com/oracle/database/jdbc/ojdbc17/23.6.0.24.10/ojdbc17-23.6.0.24.10.jar /opt/nqrust-identity/providers/ojdbc17.jar
    ADD --chown=nqrust-identity:nqrust-identity --chmod=644 https://repo1.maven.org/maven2/com/oracle/database/nls/orai18n/23.6.0.24.10/orai18n-23.6.0.24.10.jar /opt/nqrust-identity/providers/orai18n.jar
    ...
    See the [Running NQRust-Identity in a container](https://www.nqrust-identity.org/server/containers) guide for details on how to build optimized images.
    

Then continue configuring the database as described in the next section.

## [](#_configuring_a_database)Configuring a database
...
The default schema is `nqrust-identity`, but you can change it by using the `db-schema` configuration option.

It is also possible to configure the database when [Importing and exporting realms](https://www.nqrust-identity.org/server/importExport) or [Bootstrapping and recovering an admin account](https://www.nqrust-identity.org/server/bootstrap-admin-recovery):
...
For more information, see [Configuring NQRust-Identity](https://www.nqrust-identity.org/server/configuration).

## [](#_overriding_default_connection_settings)Overriding default connection settings
...
## [](#_configuring_unicode_support_for_the_database)Configuring Unicode support for the database
...
### [](#_configuring_unicode_support_for_an_oracle_database)Configuring Unicode support for an Oracle database
...
### [](#_unicode_support_for_a_microsoft_sql_server_database)Unicode support for a Microsoft SQL Server database
...
### [](#_configuring_unicode_support_for_a_mysqlmariadb_database)Configuring Unicode support for a MySQL/MariaDB database
...
```sql
create database nqrust-identity character set utf8mb3;
```
...
### [](#_configuring_unicode_support_for_a_postgresql_database)Configuring Unicode support for a PostgreSQL database
...
2.  If the default character set is not UTF 8, create the database with the UTF8 as the default character set using a command such as:
    ...
    ```
    create database nqrust-identity with encoding 'UTF8';
    ```
    

## [](#_preparing_for_postgresql)Preparing for PostgreSQL

### [](#_writer_and_reader_instances)Writer and reader instances
...
### [](#_permissions_of_the_database_user)Permissions of the database user
...
### [](#_secure_your_connection)Secure your connection
...
**Client-side Configuration:** Secure the connection by adding the following properties to your connection URL:
...
## [](#preparing-nqrust-identity-for-amazon-aurora-postgresql)Preparing for Amazon Aurora PostgreSQL

When using Amazon Aurora PostgreSQL, the [Amazon Web Services JDBC Driver](https://github.com/awslabs/aws-advanced-jdbc-wrapper) offers additional features like transfer of database connections when a writer instance changes in a Multi-AZ setup. This driver is not part of the distribution and needs to be installed before it can be used.
...
2.  When running containers: Build a custom NQRust-Identity image and add the JAR in the `providers` folder.
    ...
    See the [Running NQRust-Identity in a container](https://www.nqrust-identity.org/server/containers) guide for details on how to build optimized images, and the [Using custom NQRust-Identity images](https://www.nqrust-identity.org/operator/customizing-nqrust-identity) guide on how to run optimized and non-optimized images with the NQRust-Identity Operator.
...
While this encrypts the connection, you must still perform a full server certificate verification. To do this download the [certificate bundle](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/UsingWithRDS.SSL.html) for your AWS region.
...
## [](#_preparing_for_mysql_server)Preparing for MySQL server

Beginning with MySQL 8.0.30, MySQL supports generated invisible primary keys for any InnoDB table that is created without an explicit primary key (more information [here](https://dev.mysql.com/doc/refman/8.0/en/create-table-gipks.html)). If this feature is enabled, the database schema initialization and also migrations will fail with the error message `Multiple primary key defined (1068)`. You then need to disable it by setting the parameter `sql_generate_invisible_primary_key` to `OFF` in your MySQL server configuration before installing or upgrading NQRust-Identity.

## [](#_preparing_for_ms_sql_server)Preparing for MS SQL server
...
## [](#_changing_database_locking_timeout_in_a_cluster_configuration)Changing database locking timeout in a cluster configuration
...
## [](#_using_database_vendors_with_xa_transaction_support)Using Database Vendors with XA transaction support
...
## [](#_setting_jpa_provider_configuration_option_for_migrationstrategy)Setting JPA provider configuration option for migrationStrategy
...
For more information, check the [Migrating the database](https://www.nqrust-identity.org/docs/latest/upgrading/#_migrate_db) documentation.

## [](#_configuring_the_connection_pool)Configuring the connection pool

### [](#_mysql_and_mariadb)MySQL and MariaDB
...
## [](#multiple-datasources)Configure multiple datasources
...
You can find more details on how to connect to your own users database in the [User Storage SPI](https://www.nqrust-identity.org/docs/latest/server_development/#_user-storage-spi) documentation.
...
### [](#_required_configuration)Required configuration
...
#### [](#_1_jpa_persistence_xml_file)1\. JPA `persistence.xml` file
...
#### [](#_2_required_properties)2\. Required properties
...
### [](#_configuration_via_environment_variables)Configuration via environment variables
...
For instance, for a datasource with the name _user\_store$marketing_, you can set environment variables as follows:
...
You can find more information in the guide [Configuring NQRust-Identity](https://www.nqrust-identity.org/server/configuration), in subsection _Formats for environment variable keys with special characters_.

### [](#_backward_compatibility_for_the_quarkus_properties)Backward compatibility for the `quarkus.properties`
...
### [](#_additional_datasources_options)Additional datasources options
