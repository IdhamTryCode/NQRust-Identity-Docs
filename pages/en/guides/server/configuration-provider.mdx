...
Provider configuration options are provided when starting the server. See all support configuration sources and formats for options in [Configuring NQRust-Identity](/en/guides/server/configuration). For example via a command line option:
...
- [Configuring NQRust-Identity](/en/guides/server/configuration)
